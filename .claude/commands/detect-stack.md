...
**0. Persona Guard (run first)**

Read the active persona from `CLAUDE.md`.

- If `Active Persona: pm` or `Active Persona: designer`, respond:
  > "The {persona} persona does not consume a tech stack. `/detect-stack` is only used by engineer, data-scientist, and devops personas. Run `/start-session` instead."
  > Stop.
- If `Active Persona` is missing or empty, treat as `engineer` and continue.
