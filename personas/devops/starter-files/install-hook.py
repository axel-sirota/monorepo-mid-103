#!/usr/bin/env python3
"""Idempotently install the devops persona's PreToolUse hook into .claude/settings.json.

Usage (from monorepo root):
    python3 personas/devops/starter-files/install-hook.py

What it does:
  1. Ensures .claude/settings.json exists with a valid skeleton.
  2. Adds the devops-required Bash permissions (make/docker compose/terraform/git).
  3. Registers .claude/hooks/devops-check.sh as a PreToolUse hook on
     Bash(git commit *).
  4. Re-runs are no-ops.

Solo-safe: this script knows nothing about other personas.
"""
...
PERSONA = "devops"
HOOK_COMMAND = "${CLAUDE_PROJECT_DIR}/.claude/hooks/devops-check.sh"
...
# Per Axel's hard rules + Claude Code subagent docs: `tools:` field accepts
# tool NAMES only, not specifier patterns. Restrictions must live here.
REQUIRED_DENY = [
...
def load_settings(path: Path) -> dict:
    if not path.exists():
        return {"$schema": SCHEMA, "permissions": {"allow": [], "deny": []},
...
def main() -> int:
    ...
    hook_target = repo_root / ".claude" / "hooks" / "devops-check.sh"
