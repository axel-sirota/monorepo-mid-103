#!/usr/bin/env python3
"""Idempotently install the data-scientist persona's PreToolUse hook into .claude/settings.json.

Usage (from monorepo root):
    python3 personas/data-scientist/starter-files/install-hook.py

What it does:
  1. Ensures .claude/settings.json exists with a valid skeleton.
  2. Adds the data-scientist-required Bash permissions (make/docker/uv/git).
  3. Registers .claude/hooks/data-scientist-check.sh as a PreToolUse hook on
     Bash(git commit *).
  4. Re-runs are no-ops.

Note: the data-scientist hook depends on `jq` at runtime (for notebook
checks).  See personas/data-scientist/SETUP.md for installation.
...
HOOK_COMMAND = "${CLAUDE_PROJECT_DIR}/.claude/hooks/data-scientist-check.sh"
...
def main() -> int:
    ...
    hook_target = repo_root / ".claude" / "hooks" / "data-scientist-check.sh"
