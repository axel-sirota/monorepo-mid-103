#!/usr/bin/env python3
"""Idempotently install the PM persona's PreToolUse hook into .claude/settings.json.

Usage (from monorepo root):
    python3 personas/pm/starter-files/install-hook.py

What it does:
  1. Ensures .claude/settings.json exists with a valid skeleton.
  2. Adds the PM-required Bash permissions (git).
  3. Registers .claude/hooks/pm-check.sh as a PreToolUse hook on
     Bash(git commit *).
...
PERSONA = "pm"
HOOK_COMMAND = "${CLAUDE_PROJECT_DIR}/.claude/hooks/pm-check.sh"
...
def main() -> int:
    ...
    hook_target = repo_root / ".claude" / "hooks" / "pm-check.sh"
