#!/usr/bin/env python3
"""Idempotently install the designer persona's PreToolUse hook into .claude/settings.json.

Usage (from monorepo root):
    python3 personas/designer/starter-files/install-hook.py

What it does:
  1. Ensures .claude/settings.json exists with a valid skeleton.
  2. Adds the designer-required Bash permissions (make/npm/git/docker compose).
  3. Registers .claude/hooks/designer-check.sh as a PreToolUse hook on
     Bash(git commit *).
  4. Re-runs are no-ops.

Solo-safe: this script knows nothing about other personas.  Running it on a
fresh repo gives a working designer setup; running it after another
persona's install merges the designer hook alongside any existing entries.
"""
...
PERSONA = "designer"
HOOK_COMMAND = "${CLAUDE_PROJECT_DIR}/.claude/hooks/designer-check.sh"
...
def main() -> int:
    ...
    hook_target = repo_root / ".claude" / "hooks" / "designer-check.sh"
